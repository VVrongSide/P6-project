...
int blake2s_selftest()
{
  ...
  if (blake2s_init(&ctx, 32, NULL, 0))
    return -1;
      ...
      cout << "Calculating the keyed and unkeyed hashes.. \n";
  for (i = 0; i < 4; i++) {
    outlen = b2s_md_len[i];
    for (j = 0; j < 6; j++) {
      inlen = b2s_in_len[j];
  ...
  char stringcharthing[2400];
  cout << "\nComparing results..";
  for (i = 0; i < 32; i++) {
    // sprintf(stringcharthing, "%X  Compared to  %X", md[i], blake2s_res[i]);
    // cout << stringcharthing << "\n";
    if (md[i] != blake2s_res[i])
      return -1;
  }
  cout << "\n_______________________________________________________________\n\n";
  for (i=0; i<32; i++){
    sprintf(stringcharthing, "%X", md[i]);
    cout << stringcharthing;
  }
  cout << "\n\n                        Compared to\n\n";
  for (i=0; i<32; i++){
    sprintf(stringcharthing, "%X", blake2s_res[i]);
    cout << stringcharthing;
  }
  cout << "\n_______________________________________________________________";
  return 0;
...
int main(){
    cout << "\n\n_______________________________________________________________\n\n";
    cout << "Starting new Blake2s unit Test...\n\n";
