...
void getCiphertext(uint16_t ciphertext[]) {
  uint32_t msgKey[1];
  getMsgKey(msgKey);
//  Serial.print("msgkey: ");
//  Serial.println(msgKey[0]);

  ciphertext[0] = ciphertext[0] ^ (uint16_t)(msgKey[0] >> 16);
  ciphertext[1] = ciphertext[1] ^ (uint16_t)msgKey[0]; 
}

uint32_t getMsgKey(uint32_t msgKey[]) {
