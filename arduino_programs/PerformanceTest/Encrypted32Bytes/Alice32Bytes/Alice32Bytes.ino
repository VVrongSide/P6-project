...
//    Serial.println(payload[3]);
  ...
  uint8_t payload_b32 = payload[15];            // CHANGED to here
  ...
  LoRa.write(payload_b32);    // CHANGED to here
  LoRa.write(mic[0]);
  LoRa.write(mic[1]);
  LoRa.write(mic[2]);
  LoRa.write(mic[3]);
...
uint16_t getFirstNonce(uint16_t payload[]) {
  payload[0] = 42069;
}
...
void Expand256Block128(uint64_t K[], uint64_t roundkey[]) {     // CHANGED (NEW)
